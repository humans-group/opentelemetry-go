...
$(TOOLS)/semconvkit: PACKAGE=github.com/humans-group/opentelemetry-go/$(TOOLS_MOD_DIR)/semconvkit
...
test-coverage: | $(GOCOVMERGE)
	...
	for dir in $(ALL_COVERAGE_MOD_DIRS); do \
	  echo "$(GO) test -coverpkg=github.com/humans-group/opentelemetry-go/... -covermode=$(COVERAGE_MODE) -coverprofile="$(COVERAGE_PROFILE)" $${dir}/..."; \
	  (cd "$${dir}" && \
	    $(GO) list ./... \
	    | grep -v third_party \
	    | grep -v 'semconv/v.*' \
